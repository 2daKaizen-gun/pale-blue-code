...
Claude는 이 양식에 따라 Plan을 응답한다. Plan이 확정되면 Code 단계로 진입.
...
회고 작성 후, sub-phase를 닫기 전에 **항상** 다음 두 가지를 수행한다:
...
→ commit: feat(apod): add loading spinner during image fetch
```
...
09:30~12:00 [Light 1] API 키 발급 + .env 셋업      → commit
            [Light 2] fetch 함수 작성              → commit
            [Light 3] 응답 타입 정의               → commit

(점심)

13:00~17:00 [Light 4] APOD 컴포넌트 기본 골격      → commit
            [Light 5] 스타일링 + 반응형            → commit
            [Light 6] 로딩 스피너                  → commit
            [Light 7] 에러 처리                    → commit

17:00~17:30 [Full Test]  통합 검증
17:30~17:50 [Full Check] 회고
17:50~18:00 마일스톤 표시 + git push origin main
```

이 패턴 = 하루 7 커밋 + 1 Full Plan + 1 Full Check + 1 push.  
완벽한 *"하루 6~10 커밋"* 페이스.
...
- §9 Context Rot / Anxiety 방지 — 명시적 사이클로 결정 흐림 방지
